...
uchar sram_read_byte(unsigned int add, uchar channel)
{
	...
	{
		sram_out(((uchar)(INST_READ>>(7-i))&1), channel); // write bit 
		sram_clk(0);
		sram_clk(1);
	}
	for(i=0;i<16;i++) //Send address
	{
		sram_out(((uchar)(add>>(15-i))&1), channel); // write bit 
		sram_clk(0);		
		sram_clk(1);
	}
...
void sram_write_status(uchar val)
{
	...
	SRAM_CS_ENA;
	sram_clk(0);
	for(i=0;i<8;i++) //Send WRITE STATUS instruction
	{
		sram_out((uchar)(1>>(7-i))&1, CH_ALL); // write bit 
	...
	{
		sram_out(((uchar)(val>>(7-i))&1), CH_ALL); // write bit 
		//sram_out_all((uchar)(val>>(7-i))&1); // write bit 
		sram_clk(0);		
		sram_clk(1);
	}
...
uchar sram_read_status()
{
	...
	SRAM_CS_ENA;
	sram_clk(0);
	for(i=0;i<8;i++) //Send READ STATUS instruction
	{
		sram_out(((uchar)(5>>(7-i))&1),CH_ALL); // write bit 
		sram_clk(0);
		sram_clk(1);
	}
...
void sram_write_byte(unsigned int add, uchar val, uchar channel)
{
	...
	{
		sram_out((uchar)(INST_WRITE>>(7-i))&1, channel); // write bit 
		//sram_out_all((uchar)(INST_WRITE>>(7-i))&1); // write bit 
		sram_clk(0);
		sram_clk(1);
	}
	...
	for(i=0;i<8;i++) //Send data
	{
		sram_out(((uchar)(val>>(7-i))&1), channel); // write bit 
		//sram_out_all((uchar)(val>>(7-i))&1); // write bit 
		sram_clk(0);	
		sram_clk(1);
	}
